...
# Add some metadata
dfr <- dat[,c("dataset","species","pft","vegetation")]
...
<<plotting, echo=FALSE>>=
# natural scale
plot(1, xlim=c(0,50), ylim=c(0,1), type='n',
     xlab="Plant height (m)", ylab="Leaf mass / aboveground mass (-)")
for(i in 1:length(l))points(l[[i]]$h, l[[i]]$wfrel, type='l')
...
for(i in 1:length(l))points(log10(l[[i]]$h), log10(l[[i]]$wfrel), type='l')
...
axis(2)
for(i in 1:length(l))points(log10(l[[i]]$h), l[[i]]$wfrel, type='l')
...
# log-log scale
palette(c("blue","royalblue","red","darkorange"))
plot(1, xlim=c(-3,2), ylim=c(-3,0), type='n',
     xlab="Plant height (m)", ylab="Leaf mass / aboveground mass (-)",
     axes=FALSE)
addlogaxes(10^-3, 10^2, 10^-3, 10^0, tck1=-0.03, tck2=-0.015)
...
legend("bottomleft", c("Deciduous angiosperm","Deciduous gymnosperm",
                       "Evergreen angiosperm","Evergreen gymnosperm"),
       fill=palette())
# dev.copy2pdf(file="analysis/output/figures/LMF_height_lognat.pdf")
...
# dev.copy2pdf(file="analysis/output/figures/MF_height_loglog.pdf")
...
legend("topleft", c("Deciduous angiosperm","Deciduous gymnosperm",
                    "Evergreen angiosperm","Evergreen gymnosperm"),
       fill=palette(), inset=0.04)
# dev.copy2pdf(file="analysis/output/figures/MS_height_loglog.pdf")
@
