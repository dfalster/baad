...
if(packageVersion("smatr") < "3.4-4")
  stop("Please update smatr! We use the new quiet argument.")
...
sm <- lapply(split(dfr, dfr$pft), function(x){
      sma(log10(m.rf) ~ log10(m.lf)*dataset_species, quiet=TRUE,
           data=x)
      })
...
sm <- lapply(split(dfr, dfr$pft), function(x){
      sma(log10(m.lf) ~ log10(m.so)*dataset_species, quiet=TRUE,
           data=x)
      })
