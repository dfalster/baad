...
dir.rawData   <-  "data" # paste0() in general, but file.path here
...
for(i in 1:length(names)){
	#Aiba 2005--------------------------------------
	if(names[i]=="Aiba2005"){
		raw       <-  read.csv(paste(dir.rawData,"/",names[i],"/data.csv", sep=''), h=TRUE, stringsAsFactors=FALSE) #brings in the original .csv
	...
	if(names[i]=="Baltzer2007"){
		raw       <-  read.csv(paste(dir.rawData,"/",names[i],"/data.csv", sep=''), h=TRUE, stringsAsFactors=FALSE)
		...
		new[[i]]   <-  cbind(dataset=names[i], species=raw$species, raw[,c(4:16,18,21:24)], latitude=43.66146, longitude=-79.40006, stringsAsFactors=FALSE)
	...
	if(names[i]=="Martin1998"){
		raw        <-  read.csv(paste(dir.rawData,"/",names[i],"/data.csv", sep=''), h=TRUE, stringsAsFactors=FALSE, skip=1)
	...
	if(names[i]=="Mokany2003"){
		raw        <-  read.csv(paste(dir.rawData,"/",names[i],"/data.csv", sep=''), h=TRUE, stringsAsFactors=FALSE, skip=2)
	...
	if(names[i]=="Nouvellon2010"){
		raw        <-  read.csv(paste(dir.rawData,"/",names[i],"/data.csv", sep=''), h=TRUE, stringsAsFactors=FALSE, skip=4)
	...
	#Wang1995---------------------------------------------
	if(names[i]=="Wang1995"){
		raw          <-  read.csv(paste(dir.rawData,"/",names[i],"/data.csv", sep=''), h=TRUE, stringsAsFactors=FALSE, skip=3)
