...
if("location" %in% sitevarnames){
  sites <- sort(levels(.dat$data[,"location"]))
...
The dataset includes the following species (`r nlevels(.dat$data$species)` species in total):
`r cbind(sort(levels(.dat$data$species)))`
...
makePlotPanel(d$data, "Ishihara0000", pdf=FALSE)
```
