...
write.csv(data.frame(name=details$authors, email=details$emails, address=details$address$address[match(details$address_code, details$address$code)]), "../output/authors.csv", row.names=FALSE)
```
...
getAcknowledgments <- function(studyName) {
  ...
  out <- ""
  if(!is.na(i))
    out <- sprintf(" **%s:** %s;", studyName, metadata$Description[i])
  out
}
