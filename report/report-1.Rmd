...
printMeta  <-  function(data){
  ...
  openMeta  <-  read.csv(paste0("../",mashrDetail("dir.raw"),"/",dataset,"/studyMetadata.csv"), h=TRUE, stringsAsFactors=FALSE)
  openMeta
}
...
counts  <-  counts[names(counts)%in%mashrDetail("var.def")$Variable[mashrDetail("var.def")$Group=="tree"]]
cbind(counts)
```
